...
#                                  "indexed as of different timezones"
    ...
    @cached_property
    def profit(self):
        return pybacktest.performance.profit_summary(self.equity)    
    ...
    def plot_equity(self, subset=None, ax=None):
        ...
        window = pybacktest.parts.trades_to_equity(self.trades)
        subset = slice(window.index[0], window.index[-1])
        assert isinstance(subset, slice)
        eq = self.equity[subset].cumsum()
        
        eq.index = pandas.to_datetime(eq.index)
        range = pandas.date_range(eq.index[0],eq.index[-1])
        eq = eq.reindex(range)
        eq = eq.fillna(method='ffill')        
        ax.plot(eq.index, eq[:])
        underlying = self.ohlc.copy()
        underlying.index = pandas.to_datetime(underlying.index)
        underlying = underlying - underlying.iloc[0]
        
        ax.plot(underlying.index, underlying.O)
    ...
    def plot_trades(self, subset=None, ax=None):
        ...
        tmp = self.trades.ix[subset].copy()
        tmp['price'] = tmp['price'] - tmp.iloc[0]['price']
